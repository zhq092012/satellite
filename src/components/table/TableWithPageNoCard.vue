<template>
  <div v-bind="$attrs" class="table-with-page-no-card">
...
// 选中某一行
const toggleRowSelection = (row: any, selected?: boolean, ignoreSelectable: boolean = true) => {
  nextTick(() => {
    tbRef.value?.toggleRowSelection(row, selected)
  })
}
